...
summary(glm_female)
```
I tested for collinearity between number of breeding years and number of mates in the reduced female data set:
```{r}
#load caret
library(car)
round(vif(glm_female), digits = 3)
```
...
summary(glm_male_nonzero)
```
I tested for collinearity between mating success and number of breeding years for males.
```{r}
round(vif(glm_male_nonzero), digits = 3)
```
